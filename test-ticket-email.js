...
async function runSingleSendTest() {
  ...
  if (!targetEmail) {
    ...
    console.error("   Example: node test-ticket-email.js user@example.com");
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`Fetching user with email ${targetEmail} from the database...`);
    
    const user = await Registration.findOne({ email: targetEmail });
    
    if (!user) {
      console.log(`⚠️ No user found matching the email ${targetEmail} in the DB!`);
      return;
    }
    ...
    console.log(`🎉 Successfully sent to ${user.name} (${user.email})! Message ID: ${info.messageId}`);
